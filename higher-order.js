...
//Code Here
let evenNumbers = mixedNumbers.filter(function(even){
    return even % 2 === 0;
});
...
//Code Here
let postTaxPrices = prices.map(function(newPrice){ 
...
//Code Here
let myStrongest = monstersInYourPocket.filter(function(num, index, array){
  return num['CP'] > 200;
});
...
*/
let bobsTotal = purchases.reduce(function(acc, val, index, array){
  if(val.owner === "Bob") {
    acc += val.price
  } return acc
},0);
